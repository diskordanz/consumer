...
export class CommonService {
  ...
  listOrders(id: number) {
    return this
           .http
           .get(`${this.uri}/consumers/${id}/orders?count=10&offset=0`);
  }
  ...
  getCartItem(item : CartItem): Observable<CartItem> {
    return this
           .http
           .get<CartItem>(`${this.uri}/consumers/${item.consumer_id}/cart?product_id=${item.product.id}`);
  }
  deleteCartItem(id){
    return this
           .http
           .delete(`${this.uri}/cart/${id}`);
  }
